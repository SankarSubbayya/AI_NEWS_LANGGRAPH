"""State management using Pydantic models for type safety and validation."""
...
class TopicSummaryModel(BaseModel):
    ...
    trend_diagram_path: Optional[str] = None
...
class NewsletterContentModel(BaseModel):
    ...
    infographic_paths: List[str] = Field(default_factory=list)
...
class WorkflowState(MessagesState):
    """
    Central workflow state following metamorphosis pattern.
    Extends MessagesState for LangGraph compatibility.
    """
    # Configuration
    main_topic: str = "AI in Cancer Care"
    topics_config: Dict[str, Any] = Field(default_factory=dict)
    topics_path: Optional[str] = None
    selected_topic_names: Optional[List[str]] = None  # If None, include all topics
    ...
    def get_selected_summaries(self) -> List[TopicSummaryModel]:
        """
        Get topic summaries filtered by selected topics.
        If no selection is specified, returns all summaries.

        Returns:
            Filtered list of topic summaries
        """
        if not self.selected_topic_names:
            return self.topic_summaries

        # Filter summaries to only include selected topics
        return [
            summary for summary in self.topic_summaries
            if summary.topic_name in self.selected_topic_names
        ]

    def get_all_topic_names(self) -> List[str]:
        """Get names of all topics that have been processed."""
        return [summary.topic_name for summary in self.topic_summaries]
